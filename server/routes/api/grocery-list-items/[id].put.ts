...
export default defineHandler(async (event) => {
  ...
  const rows = await sql`
    UPDATE grocery_list_items SET
      ...
      END
    WHERE id = ${id}
    RETURNING id, list_id, name, quantity, checked, sort_order, created_at
  `;
  if (rows.length === 0) throw createError({ statusCode: 404, statusMessage: "item not found" });

  const row = rows[0] as { id: string; list_id: string; name: string; quantity: string | null; checked: boolean; sort_order: number; created_at: string };
  return {
    id: row.id,
    listId: row.list_id,
    name: row.name,
