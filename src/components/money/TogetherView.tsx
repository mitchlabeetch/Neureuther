...
export function TogetherView(
    ...
    const handleAdd = async () => {
        if (!newTitle.trim() || !newAmount || !newPaidBy || newSplitIds.length === 0)
            return;
    ...
    const handleDelete = async (id: string) => {
        await api(`/api/money/shared-expenses/${id}`, {
            method: "DELETE"
        });
    ...
    const toggleSplitUser = (userId: string, isNew: boolean) => {
        if (isNew) {
            ...
            setEditing(prev => prev ? {
                ...prev,
                splitUserIds: prev.splitUserIds.includes(userId) ? prev.splitUserIds.filter(id => id !== userId) : [...prev.splitUserIds, userId]
            } : null);
        }
    };
    ...
    if (loading) {
        return (
            <div className="px-5 pt-14 pb-4">
                <button
                    onClick={onBack}
                    className="flex items-center gap-1 text-[#b7c6c2] text-sm font-medium mb-3">
    ...
    return (
        <div className="px-5 pt-14 pb-24">
            ...
            <div className="flex items-center justify-between mb-2 animate-fade-in-up">
                <button
                    onClick={onBack}
                    className="flex items-center gap-1 text-[#b7c6c2] text-sm font-medium hover:text-[#171e19] transition-colors">
                    <ArrowLeft size={18} />Back
                            </button>
                <button
            ...
            <h2
                className="text-2xl font-semibold text-[#171e19] tracking-tight mb-4 animate-fade-in-up">Together</h2>
            {}
            <div
                className="bg-white rounded-[2.5rem] p-6 border border-[#b7c6c2]/20 shadow-[0_20px_50px_-12px_rgba(0,0,0,0.08)] mb-5 animate-fade-in-up">
                <p
                    className="text-xs font-extrabold text-[#b7c6c2] uppercase tracking-[0.1em] mb-4">Balance</p>
                    ...
                    {balances.map(b => {
                        ...
                        return (
                            <div key={b.userId} className="flex items-center justify-between">
                                ...
                                <span
                                    className={`font-bold text-lg ${b.balance >= 0 ? "text-green-600" : "text-[#ca0013]"}`}>
            ...
            <div className="space-y-3 animate-fade-in-up">
                <p
                    className="text-xs font-extrabold text-[#b7c6c2] uppercase tracking-[0.1em]">Payments</p>
                {expenses.length === 0 && (<p className="text-sm text-[#b7c6c2] text-center py-8">No expenses yet. Tap + to add one.</p>)}
                {expenses.map(exp => {
                    ...
                    return (
                        <div
                            key={exp.id}
                            className="bg-white rounded-[1.5rem] p-4 border border-[#b7c6c2]/20 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.04)]">
                            {isEditing ? (<div className="space-y-3">
                                ...
                                <div>
                                    <p
                                        className="text-[10px] font-bold uppercase tracking-[0.1em] text-[#b7c6c2] mb-1.5">Paid by</p>
                                    <div className="flex flex-wrap gap-2">
                                        {state.users.map(u => (<button
                                            ...
                                            className={`px-3 py-1.5 rounded-full text-sm font-medium transition-all ${editing!.paidBy === u.id ? "bg-[#171e19] text-white" : "bg-[#eeebe3] text-[#b7c6c2]"}`}>
                                            {u.emoji} {u.name}
                                        </button>))}
                                    </div>
                                </div>
                                <div>
                                    <p
                                        className="text-[10px] font-bold uppercase tracking-[0.1em] text-[#b7c6c2] mb-1.5">Paid for</p>
                                    <div className="flex flex-wrap gap-2">
                                        {state.users.map(u => (<button
                                            key={u.id}
                                            onClick={() => toggleSplitUser(u.id, false)}
                                            className={`px-3 py-1.5 rounded-full text-sm font-medium transition-all ${editing!.splitUserIds.includes(u.id) ? "bg-[#ca0013] text-white" : "bg-[#eeebe3] text-[#b7c6c2]"}`}>
                                            {u.emoji} {u.name}
                                        </button>))}
                                    </div>
                                </div>
                                <div className="flex gap-2 pt-1">
                                    ...
                                    <button
                                        ...
                                        className="py-2 px-4 rounded-full bg-[#eeebe3] text-[#b7c6c2] text-sm font-semibold active:scale-95 transition-transform">Cancel
                                                            </button>
                                </div>
                            </div>) : (<div className="flex items-start gap-4">
                                <div
                                    className="w-12 h-12 rounded-full bg-red-50 flex items-center justify-center shrink-0">
                                ...
                                <div className="flex-1 min-w-0">
                                    ...
                                    <div className="flex items-center gap-2 mt-1 flex-wrap">
                                        ...
                                        <span className="text-xs text-[#b7c6c2]">For: {exp.splitUserIds.map(uid => {
            ...
            {showAdd && (<div
                ...
                <div
                    ...
                    <div className="flex items-center justify-between mb-6">
                        ...
                        <button
                            onClick={() => setShowAdd(false)}
                            className="w-10 h-10 rounded-full bg-[#eeebe3] flex items-center justify-center">
                    ...
                    <div className="space-y-4">
                        <div>
                            <label
                                className="text-[10px] font-bold uppercase tracking-[0.1em] text-[#b7c6c2] block mb-1">Title</label>
                            <input
                                value={newTitle}
                        ...
                        <div className="flex gap-3">
                            ...
                            <div className="flex-1">
                                <label
                                    className="text-[10px] font-bold uppercase tracking-[0.1em] text-[#b7c6c2] block mb-1">Date</label>
                                <input
                                    type="date"
                                    value={newDate}
                        ...
                        <div>
                            <label
                                className="text-[10px] font-bold uppercase tracking-[0.1em] text-[#b7c6c2] block mb-2">Paid by</label>
                            <div className="flex flex-wrap gap-2">
                                {state.users.map(u => (<button
                                    key={u.id}
                                    onClick={() => setNewPaidBy(u.id)}
                                    className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${newPaidBy === u.id ? "bg-[#171e19] text-white" : "bg-[#eeebe3] text-[#b7c6c2]"}`}>
                                    {u.emoji} {u.name}
                                </button>))}
                            </div>
                        </div>
                        <div>
                            <label
                                className="text-[10px] font-bold uppercase tracking-[0.1em] text-[#b7c6c2] block mb-2">PAID FOR (CAN BE MULTIPLE)</label>
                            <div className="flex flex-wrap gap-2">
                                {state.users.map(u => (<button
                                    key={u.id}
                                    onClick={() => toggleSplitUser(u.id, true)}
                                    className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${newSplitIds.includes(u.id) ? "bg-[#ca0013] text-white" : "bg-[#eeebe3] text-[#b7c6c2]"}`}>
                                    {u.emoji} {u.name}
                                </button>))}
                            </div>
                        </div>
                        <button
                            ...
                            className="w-full py-4 rounded-2xl bg-[#ca0013] text-white font-bold text-base disabled:opacity-40 active:scale-[0.98] transition-all mt-2">Add Expense
                                          </button>
                    </div>
                </div>
            </div>)}
