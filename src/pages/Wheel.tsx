...
function WheelPage() {
    ...
    const handleResult = (index: number, label: string) => {
        ...
        if (isTemp) {
            ...
            const realUser = realActiveUsers.find(u => u!.emoji === segment.emoji && u!.name === segment.label);
    ...
    const handleCreate = () => {
        if (!newTitle.trim() || selectedUsers.length < 2)
            return;
    ...
    const handleEdit = () => {
        if (!activeConfig || !editTitle.trim())
            return;
    ...
    const toggleConfigUser = (configId: string, uid: string) => {
        ...
        const newUsers = config.users.includes(uid) ? config.users.filter(id => id !== uid) : [...config.users, uid];
    ...
    const lastPickUser = pendingPick && !pendingPick.userId.startsWith("temp-") ? getUserById(pendingPick.userId) : null;
    const lastPickTempUser = pendingPick && pendingPick.userId.startsWith("temp-") ? tempUsers[parseInt(pendingPick.userId.replace("temp-", ""))] : null;

    const lastPickDisplay = lastPickUser || (lastPickTempUser ? {
        ...
        <div className="app-container min-h-screen bg-[#fdf7f2] page-content">
            ...
            <div className="px-5 pt-10 pb-2">
                <div className="flex items-start justify-between">
                    <div>
                        <h1
                            className="text-2xl font-extrabold text-[#2D2B2A] font-['Brush_Script_MT',_cursive]">Spin Wheel</h1>
                        <p className="text-sm text-gray-400 font-semibold mt-0.5">Who's up for what?</p>
                    </div>
                    {}
                    {activeConfig && (<div className="flex items-center gap-0.5 mt-1">
                        {}
                            ...
                            <Tooltip>
                                <TooltipTrigger asChild>
                                    <DialogTrigger asChild>
                                        <button
                                            className="p-2 rounded-full text-gray-400 hover:text-blue-400 hover:bg-blue-50 transition-all active:scale-90">
                                            <UserPlus size={16} />
                                        </button>
                                    </DialogTrigger>
                                </TooltipTrigger>
                                <TooltipContent
                                    className="rounded-xl bg-[#2D2B2A] text-white border-none text-xs font-semibold px-3 py-2 shadow-lg">Add temporary user for this run
                                                                                                                                                                                                      </TooltipContent>
                            </Tooltip>
                                    ...
                                    <div>
                                        ...
                                        <div className="flex gap-2 mt-1.5">
                                            ...
                                            <button
                                                ...
                                                className="shrink-0 px-4 py-2 rounded-xl font-bold text-sm text-white bg-[#2D2B2A] hover:bg-[#3D3B3A] disabled:bg-gray-300 transition-all active:scale-95">Add
                                                                                                                                                                                                                                                                        </button>
                                        </div>
                                    </div>
                                    {tempUsers.length > 0 && (<div className="flex flex-wrap gap-1.5">
                                        {tempUsers.map((tu, i) => (<span
                                            key={i}
                                            className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold text-white bg-slate-400">
                                            {tu.emoji} {tu.name}
                                        </span>))}
                                    </div>)}
                                </div>
                        ...
                        <Tooltip>
                            ...
                            <TooltipContent
                                className="rounded-xl bg-[#2D2B2A] text-white border-none text-xs font-semibold px-3 py-2 shadow-lg">Edit this wheel
                                                                                                                                                                                </TooltipContent>
                        </Tooltip>
                        {}
                        <Tooltip>
                            ...
                            <TooltipContent
                                className="rounded-xl bg-[#2D2B2A] text-white border-none text-xs font-semibold px-3 py-2 shadow-lg">Delete this wheel
                                                                                                                                                                                </TooltipContent>
                        </Tooltip>
                    </div>)}
                </div>
            ...
            <div className="px-5 mb-2">
                <div className="flex gap-2 overflow-x-auto no-scrollbar pb-1">
                    {state.wheelConfigs.map(config => (<button
                        key={config.id}
                        onClick={() => {
                    ...
                    <Dialog open={showNewDialog} onOpenChange={setShowNewDialog}>
                        <Tooltip>
                            <TooltipTrigger asChild>
                                <DialogTrigger asChild>
                                    <button
                                        className="shrink-0 w-10 h-10 rounded-full text-white flex items-center justify-center shadow-md hover:shadow-lg transition-all active:scale-90 bg-[#ff9147]">
                                        <Plus size={20} />
                                    </button>
                                </DialogTrigger>
                            </TooltipTrigger>
                            <TooltipContent
                                className="rounded-xl bg-[#2D2B2A] text-white border-none text-xs font-semibold px-3 py-2 shadow-lg">Create a new wheel
                                                                                                                                                                              </TooltipContent>
                        </Tooltip>
                            ...
                            <div className="px-6 pb-6 space-y-4">
                                ...
                                <div>
                                    ...
                                    <div className="flex flex-wrap gap-2 mt-2">
                                        {state.users.map(u => (<button
                                            key={u.id}
                                            onClick={() => toggleUserSelection(u.id)}
                                            className={`flex items-center gap-1.5 px-3 py-2 rounded-xl text-sm font-bold transition-all active:scale-90 ${selectedUsers.includes(u.id) ? "text-white shadow-md" : "bg-gray-100 text-gray-500 border border-gray-200"}`}
                                ...
                                <button
                                    ...
                                    className="w-full py-3 rounded-xl font-extrabold text-white bg-[#2D2B2A] hover:bg-[#3D3B3A] disabled:bg-gray-300 disabled:text-gray-500 transition-all active:scale-[0.98]">Create Wheel
                                                                                                                                                                                                    </button>
                            </div>
                        </DialogContent>
                    </Dialog>
            ...
            {}
            {activeConfig && (<div className="px-5 mb-2 flex items-center gap-2 flex-wrap">
                <span
                    className="text-xs font-bold text-gray-400 uppercase tracking-wider font-[300] font-[Consolas,_monospace]">PEOPLE:</span>
                {realActiveUsers.map(u => (<Tooltip key={u!.id}>
                    <TooltipTrigger asChild>
                        <button
                            ...
                            className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold transition-all active:scale-90 ${activeConfig.users.includes(u!.id) ? "text-white" : "bg-gray-200 text-gray-500 line-through"}`}
                            style={activeConfig.users.includes(u!.id) ? {
                                backgroundColor: u!.color
                            } : {}}>
                            {u!.emoji} {u!.name}
                        </button>
                    ...
                    <TooltipContent
                        className="rounded-xl bg-[#2D2B2A] text-white border-none text-xs font-semibold px-3 py-2 shadow-lg">Remove {u!.name}from this wheel
                                                                                                                                    </TooltipContent>
                </Tooltip>))}
                {tempUsers.map((tu, i) => (<span
                    key={i}
                ...
                <DialogContent className="rounded-3xl max-w-[380px] mx-auto p-0 gap-0">
                    ...
                    <div className="px-6 pb-6 space-y-4">
                        ...
                        <div>
                            ...
                            <div className="flex flex-wrap gap-2 mt-2">
                                {state.users.map(u => (<button
                                    ...
                                    className={`flex items-center gap-1.5 px-3 py-2 rounded-xl text-sm font-bold transition-all active:scale-90 ${editUsers.includes(u.id) ? "text-white shadow-md" : "bg-gray-100 text-gray-500 border border-gray-200"}`}
                                    style={editUsers.includes(u.id) ? {
                                        backgroundColor: u.color
                                    } : {}}>
                                    {u.emoji} {u.name}
                                </button>))}
                        ...
                        <button
                            ...
                            className="w-full py-3 rounded-xl font-extrabold text-white bg-[#2D2B2A] hover:bg-[#3D3B3A] disabled:bg-gray-300 disabled:text-gray-500 transition-all active:scale-[0.98]">Save Changes
                                                                                                                                                        </button>
                    </div>
                </DialogContent>
            </Dialog>
            {}
            {allSegments.length >= 2 ? (<div className="px-5">
                <SpinWheel segments={allSegments} onResult={handleResult} />
            </div>) : (<div className="px-5 py-20 text-center">
            ...
            {}
            {pendingPick && lastPickDisplay && (<div className="px-5 mt-2 mb-2">
                <div
                    className={`flex items-center gap-2 bg-white rounded-2xl px-3 py-2 border shadow-sm transition-all duration-500 ${doneAnimation ? "border-green-300 bg-green-50/50 scale-105" : "border-orange-100"}`}>
                    <span
                        className="text-xs font-extrabold text-gray-400 uppercase tracking-wider font-[300]">LAST PICK</span>
                    ...
                    <Tooltip>
                        ...
                        <TooltipContent
                            className="rounded-xl bg-[#2D2B2A] text-white border-none text-xs font-semibold px-3 py-2 shadow-lg">Reset selection & temp users
                                                                                                                                                          </TooltipContent>
                    </Tooltip>
                    {!doneAnimation && !isTempPick ? (<button
                        onClick={handleDone}
                        ...
                        <Check size={14} strokeWidth={3} />Done
                                                                                                                                      </button>) : !doneAnimation && isTempPick ? (<span className="ml-auto text-xs font-semibold text-gray-400 italic">Guest pick</span>) : (<div
                        className="ml-auto flex items-center gap-1.5 text-green-500 font-extrabold text-sm animate-bounce-in">
                        <PartyPopper size={16} />+{pendingPick.points}pts!
                                                                                                                                      </div>)}
                </div>
            </div>)}
            {}
            {confirmRemoveUser && (<div
                ...
                <div
                    ...
                    <div className="flex gap-3">
                        <button
                            onClick={() => setConfirmRemoveUser(null)}
                            className="flex-1 py-3 rounded-xl font-extrabold text-[#2D2B2A] bg-gray-100 hover:bg-gray-200 transition-all active:scale-[0.98]">Cancel
                                                                                                                                                          </button>
                        <button
                            onClick={() => {
                            ...
                            className="flex-1 py-3 rounded-xl font-extrabold text-white bg-coral hover:bg-[#ff5252] transition-all active:scale-[0.98]">Yes, remove
                                                                                                                                                          </button>
                    </div>
                </div>
            </div>)}
