...
// datetime-local value, or '' for none
// '' means none
// ── Helpers ───────────────────────────────────────────────────────────
...
function LongTermChecklistPage() {
    ...
    const longTermItems = useMemo(
        () => state.checklistItems.filter(i => i.kind === "long_term" && !i.archived).sort((a, b) => {
    ...
    const handleToggleSubtask = async (task: ChecklistItem, subtask: ChecklistSubtask) => {
        ...
        if (next && task.autoCompleteOnSubtasks && !task.completed && task.kind === "long_term" && !task.archived) {
            const all = subtasksByTask.get(task.id) ?? [];

            const updated = all.map(s => s.id === subtask.id ? {
                ...s,
                completed: true
            } : s);
    ...
    const handleFlagSubmit = async () => {
        ...
        if (!/^#[0-9a-fA-F]{6}$/.test(flagForm.color))
            return;
        ...
        <div className="app-container min-h-screen bg-[#fdf7f2] page-content">
            ...
            <div className="px-5 pt-14 pb-4 animate-fade-in-up">
                ...
                <div className="flex items-center gap-3">
                    <div
                        className="w-12 h-12 rounded-[1.25rem] bg-green-50 flex items-center justify-center">
                ...
                <p
                    className="text-xs text-[#b7c6c2] font-medium mt-2.5 flex items-center gap-1.5">
            ...
            <div className="px-5 space-y-2.5 mb-3">
                        ...
                        <div
                            ...
                            <div className="p-4 flex items-center gap-3">
                                ...
                                <button
                                    ...
                                    <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                                        {deadline && (<span
                                            ...
                                            style={{
                                                color: deadline.urgency === "overdue" ? "#ca0013" : deadline.urgency === "today" || deadline.urgency === "soon" ? "#F59E0B" : "#b7c6c2"
                                            }}>
                                            {deadline.urgency === "overdue" && (<CircleAlert size={10} />)}
                                            {deadline.label}
                                        </span>)}
                                        {hasSubtasks && (<Tooltip>
                                            <TooltipTrigger asChild>
                                                <span
                                                    className="text-[10px] font-extrabold uppercase tracking-[0.08em] text-[#b7c6c2] inline-flex items-center gap-1">
                                                    <ListTree size={10} />
                                        ...
                                        {item.autoCompleteOnSubtasks && (<Tooltip>
                                            <TooltipTrigger asChild>
                                                <span
                                                    className="text-[10px] font-extrabold uppercase tracking-[0.08em] text-[#b7c6c2] inline-flex items-center gap-1">
                                                    <CircleAlert size={10} />Auto
                                                                              </span>
                                            </TooltipTrigger>
                                            <TooltipContent
                                                side="top"
                                                className="rounded-xl bg-[#171e19] text-white border-none text-[11px] font-medium px-2.5 py-1.5 shadow-lg">Completes automatically when all sub-tasks are done
                                                                        </TooltipContent>
                                        </Tooltip>)}
                                    </div>
                                </button>
                                <Tooltip>
                                    <TooltipTrigger asChild>
                                        <span
                                            className="shrink-0 inline-flex items-center gap-0.5 text-[11px] font-bold text-[#69D2A6] bg-green-50 px-2 py-0.5 rounded-full">
                                            <Star size={10} /> {item.points}
                                        </span>
                                    </TooltipTrigger>
                                    <TooltipContent
                                        side="top"
                                        className="rounded-xl bg-[#171e19] text-white border-none text-[11px] font-medium px-2.5 py-1.5 shadow-lg">Worth {item.points}points
                                                          </TooltipContent>
                                </Tooltip>
                                <button
                                    onClick={e => {
                            ...
                            {}
                            {isExpanded && (<div className="px-4 pb-4 animate-fade-in-up">
                                <div className="bg-[#eeebe3]/50 rounded-[1.25rem] p-3 space-y-2">
                                    <div className="flex items-center gap-2 mb-1 px-1">
                                        <ListTree size={12} className="text-[#b7c6c2]" />
                                        <span
                                            className="text-[10px] font-extrabold uppercase tracking-[0.12em] text-[#b7c6c2]">Sub-tasks ({doneSubtasks}/{subtasks.length})
                                                                  </span>
                                    </div>
                                    {subtasks.map(s => (<div
                                        key={s.id}
                                        ...
                                        <button
                                            ...
                                            className={`shrink-0 w-6 h-6 rounded-full border-2 flex items-center justify-center transition-all active:scale-90 ${s.completed ? "bg-[#69D2A6] border-[#69D2A6] text-white" : "border-[#b7c6c2]/40 text-transparent hover:border-[#69D2A6]"}`}>
                                            <Check size={12} strokeWidth={3} />
                                        </button>
                                        <span
                                            className={`flex-1 text-xs font-medium ${s.completed ? "text-[#b7c6c2] line-through" : "text-[#171e19]"}`}>
                                            {s.label}
                                        </span>
                                        <button
                                    ...
                                    <div className="flex items-center gap-2 pt-1">
                                        ...
                                        <Tooltip>
                                            ...
                                            <TooltipContent
                                                side="top"
                                                className="rounded-xl bg-[#171e19] text-white border-none text-[11px] font-medium px-2.5 py-1.5 shadow-lg">Add sub-task
                                                                        </TooltipContent>
                                        </Tooltip>
                                    </div>
                                </div>
                ...
                {longTermItems.length === 0 && (<div className="text-center py-8 text-[#b7c6c2] text-sm">No long-term tasks yet — tap <strong>Add new task</strong>below to
                                create one
                              </div>)}
            </div>
            {}
            <div className="px-5 mb-3">
                <button
                    ...
                    <div
                        className="w-8 h-8 rounded-full bg-[#69D2A6] flex items-center justify-center">
            ...
            <Dialog
                ...
                <DialogContent
                    className="rounded-[2rem] max-w-[420px] mx-auto p-0 gap-0 border-[#b7c6c2]/20 max-h-[90vh] overflow-y-auto">
                    ...
                    <div className="px-6 pb-6 space-y-4">
                        ...
                        <div>
                            <label className="section-header block mb-2">Deadline (optional)
                                              </label>
                            <Input
                                type="datetime-local"
                                value={form.deadline}
                            ...
                            {form.deadline && (<button
                                onClick={() => setForm(f => ({
                                    ...f,
                                    deadline: ""
                                }))}
                                className="mt-1.5 text-[11px] font-semibold text-[#b7c6c2] hover:text-[#ca0013] transition-colors">Clear deadline
                                                </button>)}
                        </div>
                        <div>
                            <label className="section-header block mb-2">Flag (optional)
                                              </label>
                            <FlagSelector
                        ...
                        <label
                            className="flex items-center gap-3 bg-[#eeebe3] rounded-xl p-3 cursor-pointer">
                            <input
                                type="checkbox"
                            ...
                            <div className="flex-1">
                                <div className="text-sm font-medium text-[#171e19]">Auto-complete when all sub-tasks are done
                                                    </div>
                                <div className="text-[11px] text-[#b7c6c2] mt-0.5">
                                </div>
                            </div>
                        </label>
                        {editingItem ? (<div className="flex gap-3 pt-1">
                            <button
            ...
            {}
            {showDeleteConfirm && editingItem && (<div
                ...
                <div
                    ...
                    <div className="space-y-2">
                        ...
                        {state.users.length > 1 && (<button
                            ...
                            <div
                                className="w-11 h-11 rounded-full flex items-center justify-center text-lg shadow-sm bg-[#69D2A6]/20">
                                <Users className="text-[#69D2A6]" size={22} />
                            </div>
                            <div className="text-left flex-1">
                                <span className="text-base font-medium text-[#171e19] block">We all participated
                                                        </span>
                                <span className="text-[11px] font-medium text-[#b7c6c2]">Award to everyone
                                                        </span>
                            </div>
                            <span
                                className="text-sm font-medium text-[#69D2A6] bg-green-50 px-2.5 py-1 rounded-full group-hover:bg-[#69D2A6] group-hover:text-white transition-all">+
                                                    {(() => {
                                    const item = state.checklistItems.find(i => i.id === activePicker.itemId);
                        ...
                        {state.users.map(u => {
                            ...
                            return (
                                <button
                                    ...
                                    <span
                                        className="ml-auto text-sm font-medium text-[#69D2A6] bg-green-50 px-2.5 py-1 rounded-full group-hover:bg-[#69D2A6] group-hover:text-white transition-all">+{points}pts
                                                            </span>
                                </button>
                            );
                        })}
            ...
            {successAnimation && (<div
                ...
                <div
                    className="animate-points-burst bg-gradient-to-br from-[#69D2A6] to-[#5BCA9B] text-white pl-3 pr-5 py-2.5 rounded-full text-lg font-extrabold shadow-2xl flex items-center gap-2 border-2 border-white/30">
                    <PartyPopper size={20} className="text-white" />+{successAnimation.points}pts!
                              </div>
            </div>)}
            {}
            {flagManagerOpen && (<div
                ...
                <div
                    ...
                    {state.taskFlags.length > 0 && (<div className="mt-6 pt-5 border-t border-[#b7c6c2]/20">
                        <h4 className="section-header mb-3">Existing flags</h4>
                        <div className="space-y-2">
                            {state.taskFlags.map(f => (<div
                                key={f.id}
                                ...
                                <Tooltip>
                                    ...
                                    <TooltipContent
                                        side="top"
                                        className="rounded-xl bg-[#171e19] text-white border-none text-[11px] font-medium px-2.5 py-1.5 shadow-lg">Edit flag
                                                                </TooltipContent>
                                </Tooltip>
                                <Tooltip>
                                    <TooltipTrigger asChild>
                                    ...
                                    <TooltipContent
                                        side="top"
                                        className="rounded-xl bg-[#171e19] text-white border-none text-[11px] font-medium px-2.5 py-1.5 shadow-lg">Delete flag
                                                                </TooltipContent>
                                </Tooltip>
                            </div>))}
                        </div>
                ...
                <div
                    ...
                    <div className="flex flex-col items-center text-center mb-5">
                        <div
                            ...
                            style={{
                                backgroundColor: flagDeleteConfirm.color + "20"
                            }}>
                            <Flag
                        ...
                        <h3 className="text-lg font-semibold text-[#171e19]">Delete this flag?
                                          </h3>
                        <p className="text-sm text-[#b7c6c2] font-medium mt-1.5 px-2">"{flagDeleteConfirm.name}" will be removed and unlinked from
                    ...
                    <div className="flex gap-3">
                        ...
                        <button
                            onClick={async () => {
                                ...
                                setForm(f => f.flagId === flagDeleteConfirm.id ? {
                                    ...f,
                                    flagId: ""
                                } : f);
        ...
        <div className="relative">
            <button
                ...
                {selected ? (<>
                    ...
                    <span className="flex-1 text-left text-sm font-medium text-[#b7c6c2]">No flag
                                    </span>
                </>)}
                <ChevronDown
                    size={14}
            ...
            {open && createPortal(<div
                ...
                {flags.map(f => (<div
                    key={f.id}
                    className={`flex items-center gap-1 rounded-xl transition-colors ${value === f.id ? "bg-[#eeebe3]" : "hover:bg-[#eeebe3]"}`}>
                    <button
                        type="button"
                        onClick={() => {
                            onChange(f.id);
                    ...
                    <Tooltip>
                        ...
                        <TooltipContent
                            side="left"
                            className="rounded-xl bg-[#171e19] text-white border-none text-[11px] font-medium px-2.5 py-1.5 shadow-lg">Edit {f.name}
                        </TooltipContent>
                    </Tooltip>
                </div>))}
